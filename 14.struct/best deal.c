...
int findm(int n,int gp[n][3],BUY schm[])
{int i,j,k,s=gp[0][0]+gp[1][1]+gp[2][2];
 schm[0].sname=1,schm[0].gprice=gp[0][0];
 schm[1].sname=2,schm[1].gprice=gp[0][1];
 schm[2].sname=3,schm[2].gprice=gp[0][2];
 for(i=0;i<n;i++)
 {for(j=0;j<n;j++)
  {for(k=0;k<n;k++)
   {if(gp[i][0]+gp[j][1]+gp[k][2]<s&&i!=j&&i!=k&&j!=k)
    {s=gp[i][0]+gp[j][1]+gp[k][2];
